...
int WifiConnect(const char *ssid, const char *psk)
{
    ...
    char serial[16] = {0};
    unsigned char mac_addr[6]={0xBC, 0x11, 0x31, 0xA4, 0x17, 0x6A};
    int ret = hi_wifi_set_macaddr((char*)mac_addr,6);
    printf("ret = %d mac_addr[0]: %02X  mac_addr[1]: %02X mac_addr[2]: %02X  mac_addr[3]: %02X mac_addr[4]: %02X  mac_addr[5]: %02X\r\n", ret ,mac_addr[0],mac_addr[1],mac_addr[2],mac_addr[3],mac_addr[4],mac_addr[5]);
    unsigned char mac[6]={0};
    hi_wifi_get_macaddr((char*)mac,6);
    // 初始化WIFI
